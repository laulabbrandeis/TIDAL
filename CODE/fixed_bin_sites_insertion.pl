...
my $USAGE = "perl fixed_bin_sites_insertion.pl inputfile.txt > test\n";
my $WINDOW_SIZE; # = 1000;
my $chrlen_file; #file with chromosome len info present (also used by FRREC)
my $freec_ratio_file;
my %option;
getopts( 'w:l:r:h', \%option );
if ( $option{w} && $option{l} && $option{r} ) {
    $WINDOW_SIZE = $option{w};
    $chrlen_file = $option{l};
    $freec_ratio_file= $option{r};
}
...
my $allchr_ratio = {}; #hash for FREEC copy number ratio
...
foreach my $chr (keys %$lenchr) {
    ...
    $allchr_ratio->{$chr}= $chr_ratio_hash;
...
#------- read the ratio from freec output -------------
open(my $RATIO, "<", $freec_ratio_file) 
    or die "unable to open file $freec_ratio_file";
...
while ( my $line = <$RATIO> ) {
    ...
    my ($chr_prefix, $start, $ratio) = split(/\t/, $line);
    my $chr = "chr".$chr_prefix;
    my $chr_ratio_hash= $allchr_ratio->{$chr};
    $chr_ratio_hash->{$start} = $ratio;
}
close $RATIO;
#-------------------------------------------
...
#   my $mod = ($coord%$WINDOW_SIZE);
    ...
    $floor = $floor + 1; #this is the key for hash, used to store and access data
...
#header= 0: chr,1: coordinate, 2: range, 3: num reads, 4: Identifiers of TE, 5: avg cr ratio, 6: freec ratio
foreach my $chr (keys %$allchr) {
	...
	my $ratio = $chr_ratio_hash->{$bin};
	print "$chr\t$low\t$key\t$val\t$str\t$cr_average\t$ratio\n";
